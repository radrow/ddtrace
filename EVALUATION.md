...
- *Listings 1* and *2* in Appendix A.1 (based on the scenario in *Figure 7*).
- *Listings 3* and *4* in Fig.19 in Appendix A.1.

*NOTE:* each individual execution of the deadlock detection test scenarios
described below may or may not deadlock, depending on (1) intrinsic
nondeterminism in the scenario itself, and (2) further nondeterminism introduced
by scheduling (similarly to the non-deterministic deadlock illustrated in
Example 3.10 in the paper). Therefore, the logs generated by some executions of
the test scenario may not report any deadlock, while other executions may show
deadlocks occurring at different times. It may **take a few retries** to obtain
a deadlock and a detection log that closely correspond to Listings 2, 3, and 4
in the paper.

*NOTE:* The output of the commands below is rather wide: to ensure correct
visualisation, you may need to run them in a maximised terminal window with a
small font. If that does not suffice, you can make their output flat by adding
the option `--indent=0` at the very end of each of the commands below.
...
#### Listings 3 an d 4 (in Figure 19)
